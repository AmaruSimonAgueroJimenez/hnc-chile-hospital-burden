...
**Chi-square test** for independence between categorical variables and mortality status:
...
**Wilcoxon rank-sum test** for comparing length of stay distribution between groups with and without each comorbidity.

### 1.4.3 Mixed-Effects Logistic Regression for Mortality

A generalized estimating equations (GEE) approach is used to account for the clustered structure of observations by year:

$$\log\left(\frac{\pi_{ij}}{1-\pi_{ij}}\right) = \beta_0 + \mathbf{x}_{ij}^T\boldsymbol{\beta} + u_j$$

where:
- $\pi_{ij}$ is the probability of mortality for patient $i$ in cluster $j$ (year)
- $\beta_0$ is the intercept
- $\mathbf{x}_{ij}$ is the vector of covariates (age, sex, service category, severity, mortality risk, comorbidities)
- $\boldsymbol{\beta}$ is the vector of regression coefficients
- $u_j \sim N(0, \sigma^2_u)$ is the random intercept for cluster $j$

An exchangeable working correlation structure is used. Models are fit separately for each of the eight primary cancer sites. Results are reported as odds ratios (OR) with 95% confidence intervals (CI).

### 1.4.4 Negative Binomial Mixed Models for Length of Stay

Length of stay is modeled as a count outcome using negative binomial regression with random intercepts by year:

$$\log(\mu_{ij}) = \beta_0 + \mathbf{x}_{ij}^T\boldsymbol{\beta} + u_j$$
$$Y_{ij} \sim \text{NB}(\mu_{ij}, \theta)$$

where:
- $Y_{ij}$ is the length of stay (in days) for patient $i$ in year $j$
- $\mu_{ij}$ is the expected length of stay
- $\theta$ is the dispersion parameter
- Other terms are as defined above

Results are reported as incidence rate ratios (IRR) with 95% CI. Models are fit separately for each primary cancer site.

### 1.4.5 Area Under the Receiver Operating Characteristic Curve (AUC)

$$\text{AUC} = \int_0^1 \text{TPR}(x) \, d\text{FPR}(x)$$

where TPR (true positive rate) and FPR (false positive rate) are computed across all classification thresholds.

### 1.4.6 Hosmer-Lemeshow Test

The Hosmer-Lemeshow statistic partitions predicted probabilities into deciles and tests goodness of fit:

$$\text{HL} = \sum_{g=1}^{G} \frac{(O_g - E_g)^2}{E_g(1 - E_g)/n_g}$$

where $O_g$ and $E_g$ are observed and expected events in decile $g$, and $n_g$ is the sample size in decile $g$. A p-value > 0.05 indicates adequate model fit.

### 1.4.7 Pseudo-R², McFadden's Coefficient of Determination

$$R^2_{\text{McF}} = 1 - \frac{\log L_f}{\log L_0}$$

where $\log L_f$ is the log-likelihood of the fitted model and $\log L_0$ is the log-likelihood of the null model.
...
where $y_i$ is the true outcome and $\hat{p}_i$ is the predicted probability.

### 1.4.10 Root Mean Squared Error (RMSE) and Mean Absolute Error (MAE)
